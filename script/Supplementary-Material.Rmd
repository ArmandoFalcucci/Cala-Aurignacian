...
subtitle: "Revisiting the Early Aurignacian in Italy: New Insights from Grotta della Cala (southern Campania)"
knit: (function(input, ...) {
    rmarkdown::render(
      input,
      output_dir = "../output"
...
```
  **Figure S2.** Photomap of the Early Aurignacian living floor at the top of spit 10 (Elaboration: S. Ricci).
...
```
  **Figure S3.** Image-based 3D model of the *Atrium series* as it was left by P. Gambassini after his excavations in 1994. On the right, the top of the Aurignacian occupation is visible, featuring a cemented surface with circular pools of varying dimensions. On the left, the trench deepens down to the marine conglomerate. (Elaboration: G. Poggi).
...
```
  **Figure S5.** State of the art after the excavation campaign conducted in 2016. The *Atrium series* lies on a marine conglomerate, which, based on geological evidence, has been hypothetically attributed to MIS5e. (Photo: Adriana Moroni).
...
```
  **Figure S6.** The unworked incisor root from AU12, square C8, from a medium-sized mammal, previously identified as a bone tool. The figure was created in Artifact3-D.
...
```
  **Figure S7.** Examples of retouched tools from spits AU13–AU10. (**a**) Endscraper on blade with stepped bilateral retouch (i.e., Aurignacian retouch); (**b**) Endscraper on flake with stepped bilateral retouch (i.e., Aurignacian retouch); (**c**) Blade with stepped bilateral retouch (i.e., Aurignacian retouch); (**d**) Carinated endscraper on a semi-cortical flake with stepped unilateral retouch; (**e**) Blade with discontinuous bilateral retouch; (**f**) Endscraper on a semi-cortical blade; (**g**) Semi-cortical flake with bilateral retouch; (**h**) Burin on truncation; (**i**) Blade with stepped bilateral retouch (i.e., Aurignacian retouch). The figure includes 3D views generated in Artifact3-D and drawings adapted from Benini et al. (1997). Artifact distribution by spit is as follows: (**f**) from AU10; (**c**) and (**h**) from AU11; (**a**), (**b**), (**d**), and (**e**) from AU12; and (**g**) and (**i**) from AU13. The numbers shown after each letter correspond to the dataset created by AF, where each artifact is prefixed with “Ca”.

### Figure S8
```{r, echo=FALSE, fig.align='center', out.width="80%"}
knitr::include_graphics("../output/figures/Figure_S8.png")
```
  **Figure S8.** Examples of burin cores. (**a**) Carinated burin from AU13; (**b**) Carinated burin from AU12; (**c**) Multiple burin from AU11. The figure includes 3D views generated in Artifact3-D and drawings adapted from Benini et al. (1997). The artifact identifiers shown after each letter correspond to the dataset created by AF, with the prefix “Ca” (the site’s standard acronym).
...
carinated.core.volume <- Dataset_cores %>%
  ...
  group_by(Spit) %>%
  ggplot(aes(x = Spit, y = Volume)) +
  geom_boxplot(aes(fill = Spit)) +
  stat_compare_means() +
  ...
  theme(text = element_text(family = "Times New Roman", size = 18),  # Apply Times New Roman font
        axis.text = element_text(family = "Times New Roman", size = 12),
        axis.title = element_text(family = "Times New Roman", size = 14),
        legend.position = "none") +
  labs(y = "Volume (cubic mm)")

# Plot for Carinated Core Flaking Surface Length
carinated.core.fsl <- Dataset_cores %>%
  filter(Carinated_y.n == "yes") %>%
  group_by(Spit) %>%
  ggplot(aes(x = Spit, y = Flaking.surface.length)) +
  geom_boxplot(aes(fill = Spit)) +
  stat_compare_means() +
  geom_jitter(shape = 16, position = position_jitter(width = 0.2), alpha = 0.2, size = 0.7) +
  # geom_jitter(shape=16, position=position_jitter(width = 0.1)) +
...
# carinated.core.volume.fsl
```
  **Figure S9.** (**a**) Boxplots with jittered points showing the distribution of the volume (in cubic millimeters) of carinated cores across the studied spits. (**b**) Boxplots with jittered points showing the distribution of the length values of the flaking surfaces of carinated cores across the studied spits. The figure includes the results of the Kruskal-Wallis tests, which are not significant in both cases.
...
```
  **Figure S10.** Examples of bipolar cores from spits AU12 (**a**, **b**) and AU13 (**c**). The figure includes 3D views generated in Artifact3-D and drawings adapted from Benini et al. (1997). The numbers shown after each letter correspond to the dataset created by AF, with each artifact prefixed by “Ca” (the site’s standard acronym).
...
Dataset_cores %>%
  ...
  mutate(Core.classification = recode(Core.classification, `Platform flake` = "Freehand", `Multidirectional` = "Freehand", `Parallel` = "Freehand", `Inclined` = "Freehand")) %>%
  dplyr::rename(`Knapping technique` = Core.classification) %>%
  ggplot(aes(x = Spit, y = log10(Volume))) +
  geom_boxplot(aes(fill = `Knapping technique`)) +
  # stat_compare_means(aes(group = `Knapping technique`), label.y = 95000) +
  # geom_jitter(shape=16, position=position_jitter(width = 0.1)) +
  ggthemes::theme_clean() +
  scale_fill_manual(values = met.brewer("Archambault", 2)) +
  theme(text = element_text(family = "Times New Roman", size = 18),  # Apply Times New Roman font to general text
        axis.text = element_text(family = "Times New Roman", size = 12),  # Apply Times New Roman font to axis text
        axis.title = element_text(family = "Times New Roman", size = 14),  # Apply Times New Roman font to axis titles
        legend.text = element_text(family = "Times New Roman", size = 12),  # Apply Times New Roman font to legend text
...
#   labs(y= "Log Volume")
```
  **Figure S11.** Comparison of the volume values of freehand cores (i.e., platform, multidirectional, parallel, and inclined) and bipolar cores in *AU13*-*AU10*, considered as a single assemblage. Volume values were logarithmically transformed to normalize the data and reduce the impact of extreme values, allowing for a more accurate comparison across core types.

<br>

### Figure S12
```{r, echo=FALSE, warning=FALSE, fig.width=8}
# Bladelet Length Raw Material Plot
...
# Bladelet Width Raw Material Plot
...
# Bladelet Thickness Raw Material Plot
...
# Arrange the three plots in a single row with labels
...
#   geom_boxplot(aes(fill = Raw.material)) +
#   stat_compare_means(comparison = comparison) +
#   stat_compare_means(label.y = 65, label.x = 1.2) +
#   geom_jitter(shape=16, position=position_jitter(width = 0.2), alpha = 0.2, size = 0.7) +
#   # geom_jitter(shape=16, position=position_jitter(width = 0.1)) +
#   ggthemes::theme_clean() +
#   scale_fill_manual(values=met.brewer("Degas", 4)) +
#   theme(text = element_text(size = 18),
#         axis.text = element_text(size = 12),
#         axis.title = element_text(size = 14),
#         legend.position = "none") +
...
#          Raw.material == "Radiolarite" | Raw.material == "Chert",
#            Class == "Blank") %>%
#   group_by(Raw.material) %>%
#   ggplot(aes(x = Raw.material, y = Width)) +
#   geom_boxplot(aes(fill = Raw.material)) +
#   stat_compare_means(comparison = comparison) +
#   stat_compare_means(label.y = 19, label.x = 1.2) +
#   # geom_jitter(shape=16, position=position_jitter(width = 0.1)) +
#   geom_jitter(shape=16, position=position_jitter(width = 0.2), alpha = 0.2, size = 0.7) +
#   ggthemes::theme_clean() +
#   scale_fill_manual(values=met.brewer("Degas", 4)) +
...
#   geom_boxplot(aes(fill = Raw.material)) +
#   stat_compare_means(comparison = comparison) +
#   stat_compare_means(label.y = 16, label.x = 1.2) +
#   geom_jitter(shape=16, position=position_jitter(width = 0.2), alpha = 0.2, size = 0.7) +
#   # geom_jitter(shape=16, position=position_jitter(width = 0.1)) +
#   ggthemes::theme_clean() +
#   scale_fill_manual(values=met.brewer("Degas", 4)) +
...
#           labels = c("A", "B", "C"), nrow=1)
```
  **Figure S12.** Boxplots showing the distribution of length (**A**), width (**B**), and thickness (**C**) across the studied spits divided according to raw material type. The figure includes the results of the Kruskal-Wallis test and pairwise comparisons.
...
```
  **Figure S13.** MCA plots. (**a**) Graph of the individuals with their associated contributions; (**b**) Scree plot showing the percentage of variance explained by the first eight dimensions; (**c**) Graph indicating the importance of the studied variables in constructing the first and second dimensions; (**d**) Contribution of the variables to the first and second dimensions. Abbreviations: ELO = Elongation, ROB = Robustness, CURV = Curvature, TORS = Torsion, DIST = Distal end shape, CS = Cross-section shape, SP = Scar pattern.
...
Dataset %>%
  ...
  dplyr::rename(`Raw material` = Raw.material) %>%
  dplyr::select(`Raw material`, Spit) %>%
  na.omit() %>%
  tabyl(`Raw material`, Spit) %>%
  adorn_totals("row") %>%
  adorn_totals("col") %>%
...
```
**Table S1.** Distribution of tools and core-tools categorized by raw material type across the studied spits. Percentages are provided in brackets.
...
Dataset_cores %>%
  ...
  # kable_classic_2(lightable_options = "striped", full_width = F)
```
  **Table S2.** Distribution of laminar cores across the studied spits, categorized by raw material type. Percentages are provided in brackets.
...
Dataset_cores %>%
  ...
  # kable_classic_2(lightable_options = "striped", full_width = F)
```
  **Table S3.** Distribution of flake and bipolar cores across the studied spits, categorized by raw material type. Rounded percentages are provided in brackets.
...
Dataset %>%
  ...
  tabyl(Spit, `Position of retouch`) %>%
  adorn_totals("col") %>%
  adorn_totals("row") %>%
  flextable() %>%
  theme_vanilla() %>%
  ...
  # kable_classic_2(lightable_options = "striped", full_width = F)
```
  **Table S4.** Position of lateral modifications on the retouched bladelets recovered from the studied spits.
...
Dataset_cores %>%
  ...
  tabyl(Spit, `Number of platforms`) %>%
  adorn_totals("col") %>%
  adorn_totals("row") %>%
  adorn_percentages("row") %>%
  adorn_pct_formatting(digits = 1) %>%
...
Dataset_cores %>%
  ...
  mutate(Core.classification = recode(Core.classification, `Platform flake` = "Platform")) %>%
  ...
  autofit()
```
  **Table S13.** Percentage of cortex coverage recorded on bipolar and flake cores, considering all spits as a single assemblage. Percentages are provided in brackets.
...
# Replace "0" with "-" in the relevant columns (AU10, AU11, AU12, AU13, and Total)
species_data[, -1] <- lapply(species_data[, -1], function(x) gsub("^0$", "-", x))

# Make species names italicized using HTML tags
# species_data$Species <- paste0("<i>", species_data$Species, "</i>")

# Display the table with the desired formatting and species names in italics
...
species_table <- flextable(species_data) %>%
  compose(j = "Species", value = as_paragraph(as_i(Species))) %>%  # Italicize the Species column
  autofit() %>%
  theme_vanilla()
...
species_table
```
  **Table S17.** Overview of the specimens characterized by evidence of human perforation recovered in the Aurignacian spits AU13–AU10 at Grotta della Cala. Percentages are reported in brackets.
