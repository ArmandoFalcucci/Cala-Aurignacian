---
title: "Tables and figures for the paper 'CALA'"
author: 
- Armando Falcucci, Keiko Kitagawa, Vincenzo Spagnolo, ..., Adriana Moroni
...
Dataset_cores %>%
  ...
  tabyl(Core.classification.3, Spit) %>%
  adorn_totals("row") %>%
  adorn_totals("col") %>%
  adorn_percentages("col") %>%
  adorn_pct_formatting(digits = 1) %>%
...
flak_surf_lenght_carinated <- Dataset_cores %>%
  ...
  select(ID, Spit, Core.classification, Flaking.surface.length) %>%
...
Figure_6 <- (comparison_bladelet_carinated.fig) / 
            (bladelet.length + bladelet.width + bladelet.thickness)

Figure_6

ggsave("../output/figures/Figure_6.tiff", plot = Figure_6, width = 10, height = 7.5, units = "in", dpi = 300)
```